...
function TaskDetails({ navigation, route }) {
  ...
  console.log('Manager' + user.Id + ' ' + task.Processor);
  const isMyTask = Boolean(user.Id === task.Processor);
  ...
  const onUpdate = () => {
    ...
    let tempTask = { ...task, Updater: user.Id };
    setTask({ ...task, Updater: user.Id });
  ...
  const onStart = () => {
    console.log(JSON.stringify(task));
    let tempTask = { ...task, Updater: user.Id, Status: 2 };
    setTask({ ...task, Updater: user.Id, Status: 2 });
    updateTaskDetails(tempTask)
      .then(res => {
        Alert.alert('Info', res.data.Message);
        // SendNoti(task.Processor, "Your task has been updated")
