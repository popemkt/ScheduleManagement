...
import { login, firebaseLogin } from "../../Services/commonServices";
...
export default function Login({ navigation }) {
  ...
  function _login(token) {
    ...
    firebaseLogin(
      ...
      (response) => {
        ...
        switch (data.RoleId) {
          ...
          case roleConstant.ROLE_NAME.Manager:
            navigation.navigate(screenConstant.SCREENS.manager, data);
            break;
          case roleConstant.ROLE_NAME.Admin:
            navigation.navigate(screenConstant.SCREENS.admin, data);
            break;
        }
      },
      (err) => {
      ...
      (final) => {
        //TODO [BHG], remove googleSignOut on connected with API
        logOut();
        authContext.setLoadingVisible(false);
      }
    );
  }
  return (
    ...
    >
      <ImageBackground source={BG_IMAGE} style={s.bgImage}>
        ...
        <View style={s.formContainer}>
          ...
          <Button title="Login with Facebook" onPress={_loginFacebook}/>
        </View>
      </ImageBackground>
    </KeyboardAvoidingView>
  );
